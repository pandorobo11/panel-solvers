"""Build and resolve the offline documentation bundled with the distribution."""
...
def build_documentation_site(project_root: Path, site_dir: Path) -> None:
    """Build the configured MkDocs site without leaving a partial destination."""
    try:
        from mkdocs.commands.build import build
        from mkdocs.config import load_config
    except ImportError as exc:
        raise RuntimeError(
            "Building panel-solvers documentation requires MkDocs. "
            "Run: uv sync --locked"
        ) from exc
...
class DocumentationSite:
    ...
    def resolve(self, page: str = "index.html") -> Path:
        ...
        target = root.joinpath(*PurePosixPath(page).parts)
    ...
    def _resolve_root(self) -> Path:
        ...
        try:
            candidate = Path(self._resources.enter_context(resources.as_file(packaged)))
        ...
        if project_root is None:
            self.close()
            raise DocumentationSiteError(
                "Bundled panel-solvers documentation site was not found. "
                "Reinstall the panel-solvers wheel."
            )
        self._temporary = tempfile.TemporaryDirectory(prefix="panel-solvers-docs-")
