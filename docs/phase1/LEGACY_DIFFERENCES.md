...
| D013 | Shield cache identity | Geometry/topology fingerprint plus direction, batch, backend | Separate geometry and centers digests plus direction, batch, backend | intentional | Both currently protect the tested cases; neutral cache identity is deferred to Phase 5. |
...
| D015 | Parallel worker logs | Worker log messages are forwarded | Worker uses a null logger | compatibility decision required | CLI/GUI logs differ under parallel runs; choose only with user-facing migration behavior. |
